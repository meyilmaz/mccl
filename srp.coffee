###
@fileOverview Coffeescript SRP implementation.
...
mccl.keyexchange.srp =
  ...
  @param {Object} group The SRP group. Use mccl.keyexchange.srp.knownGroup
  to obtain this object.
  @return {Object} A bitArray of SRP v.
  ###
  makeVerifier: (I, P, s, group) ->
    x = undefined
    x = mccl.keyexchange.srp.makeX(I, P, s)
    x = mccl.bn.fromBits(x)
    group.g.powermod x, group.N
  ...
  ###
  makeX: (I, P, s) ->
    inner = mccl.hash.sha1.hash(I + ":" + P)
    mccl.hash.sha1.hash mccl.bitArray.concat(s, inner)
  ...
  ###
  knownGroup: (i) ->
    i = i.toString()  if typeof i isnt "string"
    mccl.keyexchange.srp._initKnownGroups()  unless mccl.keyexchange.srp._didInitKnownGroups
    mccl.keyexchange.srp._knownGroups[i]
  ...
  _initKnownGroups: ->
    ...
    group = undefined
    i = 0
    while i < mccl.keyexchange.srp._knownGroupSizes.length
      size = mccl.keyexchange.srp._knownGroupSizes[i].toString()
      group = mccl.keyexchange.srp._knownGroups[size]
      group.N = new mccl.bn(group.N)
      group.g = new mccl.bn(group.g)
      i++
    mccl.keyexchange.srp._didInitKnownGroups = true
